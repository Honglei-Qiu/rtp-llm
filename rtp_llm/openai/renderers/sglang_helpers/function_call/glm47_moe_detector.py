...
class Glm47MoeDetector(BaseFormatDetector):
    ...
    def _split_complete_tool_block(self, block: str) -> Tuple[str, str]:
        ...
        if arguments_start == -1:
            # GLM-4.5/4.6 emit a literal backslash-n separator that .strip() does not remove;
            # leaving it on the name makes every call look undefined and get dropped.
            func_name = body.strip().removesuffix("\\n").strip()
            raw_arguments = ""
        else:
            func_name = body[:arguments_start].strip().removesuffix("\\n").strip()
    ...
    def detect_and_parse(self, text: str, tools: List[Tool]) -> StreamingParseResult:
        return self.detect_and_parse_with_context(text, tools)
    ...
    def parse_streaming_increment(
        ...
        while cursor < len(current_text) or self._pending_tool_buffer is not None:
            try:
                ...
                if _strict_tool_parsing():
                    raise
                logger.warning("dropping malformed streaming tool block")
                continue
            except Exception as error:
                self._buffer = ""
                self._abort_pending_tool()
                if _strict_tool_parsing():
                    raise ToolParseError("failed to parse tool call") from error
                logger.warning("dropping streaming tool block after parse error: %s", error)
                continue
