...
TEST_F(StreamCacheResourceTest, testLoadCacheDone_TransferFailureRetriesAndCanSucceed) {
    ...
    auto failed_fused_context = std::dynamic_pointer_cast<FusedAsyncReadContext>(failed_context);
    ASSERT_NE(failed_fused_context, nullptr);
    ASSERT_NE(failed_fused_context->fusedReadContext(), nullptr);
    ASSERT_EQ(failed_fused_context->fusedReadContext()->contexts().size(), 1);
    EXPECT_CALL(*mock_coord, asyncRead(testing::_))
        .Times(2)
...
TEST_F(StreamCacheResourceTest, testLoadCacheDone_TransferFailureContinuesWithoutCacheAfterRetriesExhausted) {
    prepareResource(/*reuse_cache=*/true);
    auto& resource = stream_->streamCacheResource();
...
TEST_F(StreamCacheResourceTest, testLoadCacheDone_MemoryHitRemoteMatchFailureDoesNotRetry) {
    prepareResource(/*reuse_cache=*/true);
    auto& resource = stream_->streamCacheResource();
    ...
    auto memory_match = std::make_shared<testing::NiceMock<MockAsyncMatchContext>>();
    ON_CALL(*memory_match, done()).WillByDefault(testing::Return(true));
    ON_CALL(*memory_match, success()).WillByDefault(testing::Return(true));
    ON_CALL(*memory_match, matchedBlockCount()).WillByDefault(testing::Return(2));

    const ErrorInfo remote_error(ErrorCode::CACHE_STORE_LOAD_CONNECT_FAILED, "remote cache match failed");
    auto remote_match = std::make_shared<testing::NiceMock<MockAsyncMatchContextWithError>>(remote_error);
    ON_CALL(*remote_match, done()).WillByDefault(testing::Return(true));
    ON_CALL(*remote_match, success()).WillByDefault(testing::Return(false));
    ON_CALL(*remote_match, matchedBlockCount()).WillByDefault(testing::Return(0));

    auto fused_match = std::make_shared<FusedAsyncContext>(
        std::vector<std::shared_ptr<AsyncContext>>{memory_match, remote_match});
    auto kv_resource = std::make_shared<KVCacheResource>();
    kv_resource->setMemoryReuseBlockNum(2);
    std::shared_ptr<Meta> meta;
    auto failed_context = std::make_shared<FusedAsyncReadContext>(fused_match, kv_resource, meta);
    ASSERT_TRUE(failed_context->done());
    ASSERT_FALSE(failed_context->success());
    ASSERT_EQ(failed_context->fusedReadContext(), nullptr);
    ...
    EXPECT_EQ(stream_->reuseLength(), 0);
    EXPECT_EQ(stream_->memoryReuseLength(), 0);
    EXPECT_EQ(stream_->statusInfo().code(), remote_error.code());
    EXPECT_EQ(stream_->statusInfo().ToString(), remote_error.ToString());
}
...
TEST_F(StreamCacheResourceTest, testLoadCacheDone_RetrySubmissionFailureContinuesWithoutCache) {
    prepareResource(/*reuse_cache=*/true);
    auto& resource = stream_->streamCacheResource();
