...
static bool hasP2PMatchContext(const std::shared_ptr<FusedAsyncReadContext>& read_context) {
    if (!read_context || !read_context->fusedMatchContext()) {
        return false;
    }
    for (const auto& context : read_context->fusedMatchContext()->contexts()) {
        if (std::dynamic_pointer_cast<P2PConnectorAsyncMatchContext>(context)) {
            return true;
        }
    }
    return false;
...
bool StreamCacheResource::asyncLoadCache() {
    ...
    return startAsyncLoadCache();
}

bool StreamCacheResource::startAsyncLoadCache() {
...
bool StreamCacheResource::loadCacheDone() {
    ...
    const auto match_context    = read_context ? read_context->fusedMatchContext() : nullptr;
    const bool match_succeeded  = match_context && match_context->success();
    const auto transfer_context = match_succeeded ? read_context->fusedReadContext() : nullptr;
    const bool transfer_started = transfer_context && !transfer_context->contexts().empty();
    const bool transfer_failed  = transfer_started && !transfer_context->success();
    const int  max_retry        = resource_context_.load_cache_retry_times;
    const bool is_p2p_load      = hasP2PMatchContext(read_context);
    const bool should_retry     = transfer_failed && !is_p2p_load;

    if (should_retry) {
        RTP_LLM_LOG_WARNING("load cache transfer failed after %zu transfer context(s), retry count: %d/%d, "
                            "stream: [%ld]",
                            transfer_context->contexts().size(),
                            load_cache_retry_count_,
                            max_retry,
                            stream_->streamId());
    } else if (match_context && !match_succeeded) {
        RTP_LLM_LOG_WARNING("load cache match stage failed, stream: [%ld]", stream_->streamId());
    }

    const auto error_info = load_cache_context_->errorInfo();
    load_cache_context_.reset();

    // P2P consumes its published Prefill resource on the first read, so it cannot use generic retry.
    if (is_p2p_load) {
        stream_->reportEventWithoutLock(
            StreamEvents::Error,
            error_info.hasError() ? error_info.code() : ErrorCode::LOAD_CACHE_TIMEOUT,
            error_info.hasError() ? error_info.ToString() : "P2P cache load failed");
        releaseResource();
        return true;
    }
    if (!should_retry) {
        if (error_info.hasError()) {
            stream_->reportEventWithoutLock(StreamEvents::Error, error_info.code(), error_info.ToString());
        }
        return true;
    }
    if (load_cache_retry_count_ >= max_retry) {
        // Non-P2P: reuse lengths were never applied, the blocks are still owned, and the
        // memory-cache entries are untouched on failure, so the request is fully runnable by
        // recompute. Continue without cache rather than failing a recoverable request — under
        // Prefill CP16 the memory read is a cross-worker broadcast, so one flapping worker
        // would otherwise turn every cache-hit request into a 5xx.
        RTP_LLM_LOG_WARNING("load cache failed after %d retries (transfer error), continuing without cache, "
                            "stream: [%ld]",
                            load_cache_retry_count_,
                            stream_->streamId());
        return true;
    ...
    load_cache_retry_count_++;
    if (!startAsyncLoadCache()) {
        RTP_LLM_LOG_WARNING("failed to submit cache load retry, continuing without cache, stream: [%ld]",
                            stream_->streamId());
        return true;
    }
    return false;
